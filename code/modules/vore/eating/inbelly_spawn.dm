...
"},"OOC Warning")			// Warning.
	...
	for(var/mob/living/pred in living_mob_list)
		...
		if(ishuman(pred))
			...
			if(!H.latejoin_vore) //TORCHEdit - Changes pref to the same as vorespawn pred
				continue
			eligible_targets += H
			continue
		if(issilicon(pred))
			...
			if(!S.latejoin_vore) //TORCHEdit - Changes pref to the same as vorespawn pred
				continue
			eligible_targets += S
			continue
		if(istype(pred, /mob/living/simple_mob))
			...
			if(!SM.vore_active)						// No vore, no bellies, no inbelly spawning
				continue
			if(!SM.latejoin_vore) //TORCHEdit - Changes pref to the same as vorespawn pred
				continue
			eligible_targets += SM
			continue
...
/mob/living/proc/inbelly_spawn_prompt(client/potential_prey)
	...
	var/answer = tgui_alert(src, "[potential_prey.prefs.real_name] wants to spawn in one of your bellies. Do you accept?", "Inbelly Spawning", list("Yes", "No")) //TORCHEdit - hides ckey
	if(answer != "Yes")
