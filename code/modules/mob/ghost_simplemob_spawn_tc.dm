...
GLOBAL_LIST_INIT(ghost_spawnable_mobs,list(
	...
	// "Deer" = /mob/living/simple_mob/vore/deer, // TORCHEdit - We don't have deer mobs in Torch. We should uncomment this if ported later
	"Defanged Xenomorph" = /mob/living/simple_mob/vore/xeno_defanged,
	// "Doglin" = /mob/living/simple_mob/vore/doglin/basic, // TORCHEdit - We don't have doglin in Torch. We should uncomment this if ported later
...
/mob/observer/dead/verb/join_as_simplemob()	//Copypasta from join_as_drone()
	...
	if(!(CONFIG_GET(flag/allow_ghost_mob_spawn)))
	...
	if(GLOB.ghost_mob_spawn_count >= CONFIG_GET(number/ghost_mob_count))
...
/datum/admins/proc/add_ghost_mob_spawns()
	...
	var/amount = tgui_input_number(usr, "How many mobs should ghosts be able to spawn as?", "How many mobs", CONFIG_GET(number/ghost_mob_count))
	if(amount)
		CONFIG_SET(number/ghost_mob_count, amount)
	else
